...
cap.set(4, 1080)  # Set property 4 (frame length)
...
while True:
    ...
    if currentTime < nextReadingTime:
        # Display a message or continue showing the current frame
        if webcam:
            success, img = cap.read()
            print("Webcam Successfull")
        else:
            img = cv2.imread(path)
    ...
    if len(contours) != 0:
        ...
        img2, contours2 = utils.getContours(imgWarp,
                                            cannyThreshold=[50, 50],  # Threshold for certainty
        ...
        if len(contours) != 0:
            for obj in contours2:
                ...
                cv2.arrowedLine(img2, (newPoint[0][0][0], newPoint[0][0][1]), (newPoint[1][0][0], newPoint[1][0][1]),
                                (255, 0, 255), 3, 8, 0, 0.05)
                cv2.arrowedLine(img2, (newPoint[0][0][0], newPoint[0][0][1]), (newPoint[2][0][0], newPoint[2][0][1]),
                                (255, 0, 255), 3, 8, 0, 0.05)
                x, y, w, h = obj[3]
                cv2.putText(img2, '{}cm'.format(newWidth), (x + 30, y - 10), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5,
                            (255, 0, 255), 2)
                cv2.putText(img2, '{}cm'.format(newHeigth), (x - 70, y + h // 2), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5,
                            (255, 0, 255), 2)

            nextReadingTime = currentTime + 5
